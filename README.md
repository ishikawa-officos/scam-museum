...
| `npm run check-branches` | 選択の結果と、その後の会話が食い違っていないか検査（後述） |
...
- **ビートを合流させたら `npm run check-branches` を回す。** 下記参照

### 選択の結果と、その後の会話がずれていないか

複数の選択肢が同じビートに入るのは設計どおりで、枝を全部持つと維持できない。
問題になるのは、合流した先の本文が**特定の経路を通ってきたことを前提に**書かれている場合。

第4展示室で実際に起きた例：

```
n07 で「これは詐欺ではないですか？」と書かずに進んでも、
翌朝みかが「昨日はちょっとびっくりしましたね」と話しかけてくる。
```

何も起きていないのに、起きた前提で会話が続いていた。
直すときは、そのビートを経路ごとに分ける（`n09` と `n09-after`）。

```bash
npm run check-branches
```

合流地点のうち、本文に「昨日」「さっき」「あの話」など**前を指す言葉**が
入っているものを報告する。読んで問題なければ
`scripts/check-branches.mjs` の `REVIEWED` に理由を書いて登録する。
登録済みは報告されないので、新しく増えたぶんだけが目に入る。

**この検査で見つからないもの**：見ているのは「前を指す言葉があるか」だけなので、
逆向き（何か起きた経路で「誰も何も疑っていない」と書いてしまう）は素通りする。
そちらは人が読んで気づくしかない。
