...
`og:image` のURLには、ビルド時に中身のハッシュが付く（`/ogp.png?v=8a00b0f4`）。
画像を作り直せばURLも変わるので、**画像単位のキャッシュ**は自動的に外れる。

ただし **LINE はページURL単位でOGPを丸ごとキャッシュする**ので、
一度貼ったURLは画像を差し替えても更新されない。確認するときは、
`https://.../?v=2` のように相手が見たことのないURLで貼り直すこと。
サイトの表示は変わらない。
