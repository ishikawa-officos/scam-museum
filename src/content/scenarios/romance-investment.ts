...
export const romanceInvestment: Scenario = {
...

    {
      ...
      choices: [
        ...
        },
        {
          ...
          sentAs: '（全額の出金手続きをした）',
          outcomeHint:
            '入れた5万円より多い64,800円が戻ります。この段階なら相手は止めません。回収はもっと大きな入金のあとに行われるためです。勝てますが、勝った記憶が次を呼びます。',
          effects: { days: 2 },
          next: 'b20',
        },
      ],
      ...
      choices: [
        {
          ...
          // 画面の評価額は 64,800。入れたのは5万円。
          // 以前は5万円しか戻していなかったので、14,800円が消えていた
          effects: { damage: -64_800, days: 3 },
          next: { ending: 'withdrew-early' },
        },
        {
          id: 'c2',
          label: '出金できたなら本物だ。もう一度やってみる',
          effects: { damage: -64_800, trust: 15, days: 3 },
  ...
  endings: [
    ...
    },
    {
      id: 'withdrew-early',
      grade: 'B_LUCKY',
      title: 'あなたは詐欺師からお金を奪ってしまいました',
      body: [
        '5万円を入れて、64,800円を引き出しました。**14,800円の黒字**です。勝ち逃げです。',
        'これは演出ではありません。相手は本当に14,800円を損しています。回収はもっと大きな入金のあとなので、この段階の出金は通します。**ここで引き上げれば、詐欺師に損をさせることは実際にできてしまいます。**',
        'ところが、危ないのは負けたときではありません。**勝ったときです。**「出金できた」という喜びは、そのまま「次もやれる」に変わります。相手が狙っているのは14,800円ではなく、次にあなたが自分から戻ってくることのほうです。',
        'ですから、ここで勝って、勝ち逃げしてください。**この一回で終わりにしてください。**',
        '次に同じような話に出会ったときは、相手にしないでください。「どうせ詐欺だから、こちらが奪ってやろう」とは考えないでください。手口は日々巧妙になります。勝った経験があるぶんだけ、次は深く入ってしまいます。**勝ちの喜びにとらわれた人は、大負けするまで降りられません。**',
